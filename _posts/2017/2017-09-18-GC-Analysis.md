...
简单总结一下，Java GC就是通过GC收集器回收不在存活的对象，保证JVM更加高效的运转。如果不了解GC算法和垃圾回收器可以参考这篇文章：[jvm系列(三):GC算法 垃圾收集器](http://www.intelyes.xyz/jvm/2017/08/29/GC-garbage-collection.html)。
...
更详细的内容参考这篇文章：[jvm系列(四):jvm调优-命令篇](http://www.intelyes.xyz/jvm/2017/09/03/jvm-command.html)
