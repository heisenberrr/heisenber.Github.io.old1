...
``` sh
docker@manager1:~$ docker service create --replicas 1 --name helloworld alpine ping intelyes.xyz
...
- `--replicas` 设置启动的示例数
- `alpine `指的是使用的镜像名称，`ping intelyes.xyz`指的是容器运行的bash
...
96bf5b1d8010        alpine:latest       "ping intelyes.xyz"   4 minutes ago       Up 4 minutes                            helloworld.1.rgroe3s9qa53lf4u4ky0tzcb8
```
